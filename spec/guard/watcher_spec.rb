...
describe Guard::Watcher do
    ...
    context "with a watcher without action" do
      context "that is a regex pattern" do
        ...
        it "returns the paths that matches the regex" do
          expect(described_class.match_files(@guard, ['guard_rocks_spec.rb', 'guard_rocks.rb'])).to eq(['guard_rocks_spec.rb'])
        end
      end
    ...
    context "with a watcher action without parameter" do
      context "for a watcher that matches file strings" do
        ...
        it "returns a single file specified within the action" do
          expect(described_class.match_files(@guard, ['spec_helper.rb'])).to eq(['spec'])
        end

        it "returns multiple files specified within the action" do
          expect(described_class.match_files(@guard, ['hash.rb'])).to eq(['foo', 'bar'])
        end

        it "returns multiple files by combining the results of different actions" do
          expect(described_class.match_files(@guard, ['spec_helper.rb', 'array.rb'])).to eq(['spec', 'foo', 'bar'])
        end

        it "returns nothing if the action returns something other than a string or an array of strings" do
          expect(described_class.match_files(@guard, ['addition.rb'])).to eq([])
        end

        it "returns nothing if the action response is empty" do
          expect(described_class.match_files(@guard, ['blank.rb'])).to eq([])
        end

        it "returns nothing if the action returns nothing" do
          expect(described_class.match_files(@guard, ['uptime.rb'])).to eq([])
        end
      end
      ...
      context 'for a watcher that matches information objects' do
        ...
        it "returns a single file specified within the action" do
          expect(described_class.match_files(@guard_any_return, ['spec_helper.rb']).class).to be Array
          expect(described_class.match_files(@guard_any_return, ['spec_helper.rb']).empty?).to be_falsey
        end

        it "returns multiple files specified within the action" do
          expect(described_class.match_files(@guard_any_return, ['hash.rb'])).to eq([{:foo => 'bar'}])
        end

        it "returns multiple files by combining the results of different actions" do
          expect(described_class.match_files(@guard_any_return, ['spec_helper.rb', 'array.rb'])).to eq(['spec', ['foo', 'bar']])
        end

        it "returns the evaluated addition argument in an array" do
          expect(described_class.match_files(@guard_any_return, ['addition.rb']).class).to be Array
          expect(described_class.match_files(@guard_any_return, ['addition.rb'])[0]).to eq 2
        end

        it "returns nothing if the action response is empty string" do
          expect(described_class.match_files(@guard_any_return, ['blank.rb'])).to eq([''])
        end

        it "returns nothing if the action returns empty string" do
          expect(described_class.match_files(@guard_any_return, ['uptime.rb'])).to eq([''])
        end
      end
    end
    ...
    context "with a watcher action that takes a parameter" do
      context "for a watcher that matches file strings" do
         ...
         it "returns a substituted single file specified within the action" do
           expect(described_class.match_files(@guard, ['lib/my_wonderful_lib.rb'])).to eq(['spec/my_wonderful_lib_spec.rb'])
         end
         ...
         it "returns multiple files by combining the results of different actions" do
           expect(described_class.match_files(@guard, ['lib/my_wonderful_lib.rb', 'array.rb'])).to eq(['spec/my_wonderful_lib_spec.rb', 'foo', 'bar'])
         end
      ...
      context "for a watcher that matches information objects" do
        ...
        it "returns a substituted single file specified within the action" do
          expect(described_class.match_files(@guard_any_return, ['lib/my_wonderful_lib.rb'])).to eq(['spec/my_wonderful_lib_spec.rb'])
        end

        it "returns a hash specified within the action" do
          expect(described_class.match_files(@guard_any_return, ['hash.rb'])).to eq([{:foo => 'bar', :file_name => 'hash.rb'}])
        end

        it "returns multiple files by combining the results of different actions" do
          expect(described_class.match_files(@guard_any_return, ['lib/my_wonderful_lib.rb', 'array.rb'])).to eq(['spec/my_wonderful_lib_spec.rb', ['foo', 'bar', "array.rb"]])
        end

        it "returns the evaluated addition argument + the path" do
          expect(described_class.match_files(@guard_any_return, ['addition.rb'])).to eq(["2addition.rb"])
        end
  ...
  describe ".match_files?" do
    ...
    context "with a watcher that matches a file" do
      specify { expect(described_class.match_files?(@guards, ['lib/my_wonderful_lib.rb', 'guard_rocks_spec.rb'])).to be_truthy }
    end

    context "with no watcher that matches a file" do
      specify { expect(described_class.match_files?(@guards, ['lib/my_wonderful_lib.rb'])).to be_falsey }
    end
  end
  ...
  describe ".match_guardfile?" do
    before { allow(Guard::Dsl).to receive(:guardfile_path) { Dir.pwd + '/Guardfile' } }
