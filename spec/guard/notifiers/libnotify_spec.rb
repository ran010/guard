...
describe Guard::Notifier::Libnotify do
  ...
  describe '.available?' do
    context 'without the silent option' do
      it 'shows an error message when not available on the host OS' do
        expect(::Guard::UI).to receive(:error).with 'The :libnotify notifier runs only on Linux, FreeBSD, OpenBSD and Solaris.'
      ...
      it 'shows an error message when the gem cannot be loaded' do
        ...
        expect(::Guard::UI).to receive(:error).with "Please add \"gem 'libnotify'\" to your Gemfile and run Guard with \"bundle exec\"."
        expect(subject).to receive(:require).with('libnotify').and_raise LoadError
        subject.available?
      end
    end
    ...
    context 'with the silent option' do
      it 'does not show an error message when not available on the host OS' do
        expect(::Guard::UI).not_to receive(:error).with 'The :libnotify notifier runs only on Linux, FreeBSD, OpenBSD and Solaris.'
        expect(RbConfig::CONFIG).to receive(:[]).with('host_os').and_return 'darwin'
      ...
      it 'does not show an error message when the gem cannot be loaded' do
        expect(RbConfig::CONFIG).to receive(:[]).with('host_os').and_return 'linux'
        expect(::Guard::UI).not_to receive(:error).with "Please add \"gem 'libnotify'\" to your Gemfile and run Guard with \"bundle exec\"."
        expect(subject).to receive(:require).with('libnotify').and_raise LoadError
        subject.available?(true)
      end
    end
  ...
  describe '.notify' do
    it 'requires the library again' do
      expect(subject).to receive(:require).with('libnotify').and_return true
